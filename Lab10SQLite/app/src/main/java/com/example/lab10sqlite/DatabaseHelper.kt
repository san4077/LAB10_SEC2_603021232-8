...
class DatabaseHelper (context: Context) : SQLiteOpenHelper (context, DB_NAME, null, DB_VERSION) {
    override fun onCreate(db: SQLiteDatabase?) {
        val CREATE_TABLE = "CREATE TABLE $TABLE_NAME" + "($COLUMN_ID TEXT PRIMARY KEY, $COLUMN_NAME TEXT," +
                "$COLUMN_AGE Integer)"
    ...
    fun insertStudent(std : Student) : Long{
        ...
        val succ = db.insert(TABLE_NAME, null, values)
        db.close()
        return succ
    ...
    fun updateStudent(std: Student): Int{
        ...
        val succ = db.update(TABLE_NAME, values,"$COLUMN_ID = ?", arrayOf(std.id))
        db.close()
        return succ

    }

    fun deleteStudent(std_id: Student): Int{
        val db = writableDatabase
        val succ = db.delete(TABLE_NAME,"$COLUMN_ID = ?", arrayOf(std_id.id))
        db.close()
        return succ
